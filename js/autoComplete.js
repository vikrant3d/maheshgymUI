...
function getCustomerDetails(value){
    ...
    var myid = value.substring(index+1);
    var map = getFormData("#customerDetails");
	  map["id"]=myid;
		$.ajax({
          ...
          if($(response1).attr('transactionList') != null){		  
            $.each($(response1).attr('transactionList'), function(key,response) {	
              var newRow = "<tr><td>"+ ++key +"</td><td>"+$(response).attr('paymentDescription')+"</td></tr>";
              $("#custDetailsTable tbody").append(newRow);
					...
					$('#custDetailsTable').DataTable({
						...
						fetchImageDetails(myid,'image-sec')
				}
			});
}
...
$.ajax({
  type: 'POST',
  url: contextPath +"getAllSearchCustomers",
  success: function (response1) { 
